...
export const getCourseDiscussions = async (courseId: string) => {
  const { data, error } = await supabase
    ...
    .select('*, author:user_profiles!academy_discussions_author_id_fkey(username, display_name, avatar_url)')
    .eq('course_id', courseId)
    .is('parent_id', null)
    .order('is_pinned', { ascending: false })
    .order('created_at', { ascending: false });
  ...
  return (data || []).map((d: any) => ({
    ...d,
    author_name: d.author?.display_name || d.author?.username,
    author_avatar: d.author?.avatar_url,
  }));
};

export const getDiscussionReplies = async (parentId: string) => {
  const { data, error } = await supabase
    .from('academy_discussions')
    .select('*, author:user_profiles!academy_discussions_author_id_fkey(username, display_name, avatar_url)')
...
export const createDiscussion = async (discussion: { course_id: string; author_id: string; title: string; content: string; parent_id?: string }) => {
  const { data, error } = await supabase
    .from('academy_discussions')
    .insert(discussion)
    .select()
...
// MESSAGES
// ============================================================
export const getConversation = async (userId: string, otherUserId: string, courseId?: string) => {
  let query = supabase
    .from('academy_messages')
    .select('*, sender:user_profiles!academy_messages_sender_id_fkey(username, display_name, avatar_url), recipient:user_profiles!academy_messages_recipient_id_fkey(username, display_name, avatar_url)')
...
export const sendMessage = async (message: { sender_id: string; recipient_id: string; course_id?: string; subject?: string; content: string }) => {
  const { data, error } = await supabase
    .from('academy_messages')
    .insert(message)
    .select()
...
export const addTeacherCredential = async (credential: { teacher_id: string; credential_type: string; title: string; issuing_organization?: string; issue_date?: string; expiry_date?: string; credential_id?: string; document_url?: string }) => {
  const { data, error } = await supabase
    .from('academy_teacher_credentials')
    .insert(credential)
    .select()
...
export const verifyCredential = async (credentialId: string, verifiedBy: string) => {
  const { error } = await supabase
    .from('academy_teacher_credentials')
...
// TEACHER PAYOUTS
// ============================================================
export const getTeacherPayouts = async (teacherId: string) => {
  const { data, error } = await supabase
...
export const requestPayout = async (teacherId: string, amount: number) => {
  const { data, error } = await supabase
    .from('academy_teacher_payouts')
    .insert({ teacher_id: teacherId, amount, payout_type: 'enrollment', status: 'pending' })
    .select()
    .single();
  if (error) throw error;
  return data;
...
// LOAN PAYMENTS
// ============================================================
export const getLoanPayments = async (studentId: string) => {
  const { data, error } = await supabase
...
export const makeLoanPayment = async (studentId: string, enrollmentId: string, amount: number) => {
  const { data, error } = await supabase
    .from('academy_loan_payments')
    .insert({ student_id: studentId, enrollment_id: enrollmentId, amount, payment_type: 'manual', status: 'completed' })
    .select()
    .single();
  if (error) throw error;
  return data;
...
// ACCREDITATION
// ============================================================
export const getAccreditationOrgs = async () => {
  const { data, error } = await supabase
...
export const getAccreditationRequests = async (courseId?: string) => {
  let query = supabase
    .from('academy_accreditation_requests')
    .select('*, course:academy_courses(name), org:academy_accreditation_orgs(name)')
    .order('created_at', { ascending: false });
  if (courseId) query = query.eq('course_id', courseId);
  const { data, error } = await query;
  if (error) throw error;
...
export const submitAccreditationRequest = async (request: { course_id: string; teacher_id: string; org_id?: string; request_notes?: string }) => {
  const { data, error } = await supabase
    .from('academy_accreditation_requests')
    .insert(request)
    .select()
...
export const getCourseReviews = async (courseId: string) => {
  const { data, error } = await supabase
    ...
    .select('*, student:user_profiles!academy_course_reviews_student_id_fkey(username, display_name, avatar_url)')
    .eq('course_id', courseId)
    .eq('is_public', true)
    .order('created_at', { ascending: false });
  if (error) throw error;
  return (data || []).map((r: any) => ({
    ...r,
    student_name: r.student?.display_name || r.student?.username,
    student_avatar: r.student?.avatar_url,
  }));
};

export const submitCourseReview = async (review: { course_id: string; student_id: string; enrollment_id?: string; rating: number; review?: string }) => {
  const { data, error } = await supabase
    .from('academy_course_reviews')
    .upsert(review)
    .select()
...
// PATHWAY ENROLLMENTS
// ============================================================
export const enrollInPathway = async (studentId: string, pathwayId: string) => {
  const { data, error } = await supabase
    .from('academy_pathway_enrollments')
    .insert({ student_id: studentId, pathway_id: pathwayId, status: 'active' })
    .select()
    .single();
  if (error) throw error;
  return data;
...
export const getPathwayEnrollments = async (studentId: string) => {
  const { data, error } = await supabase
    .from('academy_pathway_enrollments')
    .select('*, pathway:academy_learning_pathways(*)')
    .eq('student_id', studentId)
    .order('created_at', { ascending: false });
  if (error) throw error;
  return data || [];
...
export const updatePathwayProgress = async (enrollmentId: string, courseIndex: number) => {
  const { error } = await supabase
    .from('academy_pathway_enrollments')
    .update({ current_course_index: courseIndex })
    .eq('id', enrollmentId);
  if (error) throw error;
};
...
export const generateCertificate = async (enrollmentId: string) => {
  const { data: enrollment } = await supabase
    .from('academy_enrollments')
    .select('*, course:academy_courses(name), student:user_profiles(display_name, username)')
    .eq('id', enrollmentId)
    .single();
  if (!enrollment) throw new Error('Enrollment not found');

  const certNumber = `CERT-${new Date().getFullYear()}-${Date.now().toString(36).toUpperCase()}`;
  const verificationId = `V-${Math.random().toString(36).substring(2, 10).toUpperCase()}`;
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      teacher_id: enrollment.academy_courses?.teacher_id,
  ...
  await supabase
    .from('academy_enrollments')
    .update({ certificate_issued: true, certificate_id: data.id })
    .eq('id', enrollmentId);
...
// COMPLETION ENGINE
// ============================================================
export const calculateProgress = async (studentId: string, courseId: string) => {
  const settings = await getAcademySettings();
  const attendanceWeight = parseInt(settings.attendance_weight || '20') / 100;
  const assignmentWeight = parseInt(settings.assignment_weight || '40') / 100;
  const quizWeight = parseInt(settings.quiz_weight || '20') / 100;
  const examWeight = parseInt(settings.exam_weight || '20') / 100;

  const attendancePct = await getAttendancePercentage(studentId, courseId);
  ...
  if (assignments && assignments.length > 0) {
    ...
    if (submissions && submissions.length > 0) {
      ...
      const totalMax = submissions.reduce((s: number, sub: any) => s + (sub.max_points || 100), 0);
      assignmentPct = totalMax > 0 ? (totalScore / totalMax) * 100 : 100;
    } else {
      assignmentPct = 0;
    }
  }
  ...
  if (quizzes && quizzes.length > 0) {
    const quizIds = quizzes.filter((q: any) => q.quiz_type !== 'exam' && q.quiz_type !== 'assessment').map((q: any) => q.id);
    const examIds = quizzes.filter((q: any) => q.quiz_type === 'exam' || q.quiz_type === 'assessment').map((q: any) => q.id);
    ...
    if (quizIds.length > 0) {
      ...
      if (quizAttempts && quizAttempts.length > 0) {
        quizPct = quizAttempts.reduce((s: number, a: any) => s + (a.percentage || 0), 0) / quizAttempts.length;
      } else {
        quizPct = 0;
      }
    }
    ...
    if (examIds.length > 0) {
      ...
      if (examAttempts && examAttempts.length > 0) {
        examPct = examAttempts.reduce((s: number, a: any) => s + (a.percentage || 0), 0) / examAttempts.length;
      } else {
        examPct = 0;
      }
    }
  ...
  const overall = Math.round(
    attendancePct * attendanceWeight +
    assignmentPct * assignmentWeight +
    quizPct * quizWeight +
    examPct * examWeight
  );

  return { attendancePct, assignmentPct, quizPct, examPct, overall };
};

export const checkAndCompleteCourse = async (studentId: string, courseId: string) => {
  const { overall, attendancePct, assignmentPct, quizPct, examPct } = await calculateProgress(studentId, courseId);

  const { data: enrollment } = await supabase
    .from('academy_enrollments')
    .select('id, minimum_attendance_pct')
    .eq('student_id', studentId)
    .eq('course_id', courseId)
    .eq('status', 'accepted')
    .maybeSingle();

  if (!enrollment) return { completed: false, reason: 'No active enrollment' };

  const minAttendance = enrollment.minimum_attendance_pct || 80;
  const settings = await getAcademySettings();
  const autoComplete = settings.auto_complete_enabled === 'true';

  await supabase.from('academy_completion_log').insert({
  ...
  await supabase
    .from('academy_enrollments')
    .update({ progress_pct: overall })
    .eq('id', enrollment.id);

  if (autoComplete && overall >= 70 && attendancePct >= minAttendance) {
    const letterGrade = overall >= 90 ? 'A' : overall >= 80 ? 'B' : overall >= 70 ? 'C' : overall >= 60 ? 'D' : 'F';
    await supabase
      .from('academy_enrollments')
      .update({
        status: 'completed',
    ...
    return { completed: true, overall, grade: letterGrade };
  }

  return { completed: false, overall, attendancePct, assignmentPct, quizPct, examPct };
};

// ============================================================
// TEACHER MANAGEMENT
// ============================================================
export const suspendTeacher = async (teacherId: string, suspendedBy: string, reason: string) => {
  const { error } = await supabase
    .from('academy_teachers')
    .update({
      is_active: false,
...
// ============================================================
export const approveCourse = async (courseId: string, approvedBy: string) => {
  const { error } = await supabase
    .from('academy_courses')
    .update({ admin_approved: true, admin_approved_by: approvedBy, status: 'published' })
    .eq('id', courseId);
  if (error) throw error;
};
...
export const getPendingCourses = async () => {
  const { data, error } = await supabase
    .from('academy_courses')
    .select('*, teacher:academy_teachers(teacher_id, teacher_user:user_profiles(display_name, username)), category:academy_categories(name)')
