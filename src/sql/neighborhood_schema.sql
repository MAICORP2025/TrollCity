...
-- 15. Insurance Broker Applications table
CREATE TABLE IF NOT EXISTS insurance_broker_applications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
...
ALTER TABLE insurance_broker_applications ENABLE ROW LEVEL SECURITY;
