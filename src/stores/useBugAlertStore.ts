...
export const useBugAlertStore = create<BugAlertState>((set, get) => ({
  ...
  addAlert: (alert) => {
    ...
    const newAlerts = [alert, ...alerts.filter(a => a.id !== alert.id)];
    const newActiveAlerts = alert.status === 'active'
      ? [alert, ...activeAlerts.filter(a => a.id !== alert.id)]
      : activeAlerts.filter(a => a.id !== alert.id);
    const newUnreadAlerts = alert.status === 'active'
      ? [alert, ...unreadAlerts.filter(a => a.id !== alert.id)]
  ...
  fetchAlerts: async (filters = {}) => {
    const effectiveFilters = { status: filters.status ?? 'active', ...filters };
