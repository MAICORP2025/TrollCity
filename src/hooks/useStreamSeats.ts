...
export function useStreamSeats(streamId: string | undefined, userId?: string, broadcasterProfile?: any, streamData?: any) {
  ...
  const joinSeat = async (seatIndex: number, price: number) => {
    ...
    // Prevent broadcaster from joining seats other than their own (seat 0) when they are the host
    if (streamData?.user_id === effectiveUserId && seatIndex !== 0) {
      toast.error("As the broadcaster, you can only occupy seat 0");
      return false;
    }
