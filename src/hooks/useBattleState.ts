...
export function useBattleState({ streamId, localUserId, isHost, hostId }: UseBattleStateProps) {
  ...
  const parseBattleDate = (battle: any, ...fields: string[]): Date | null => {
    for (const field of fields) {
      if (battle?.[field]) {
        return new Date(battle[field]);
      }
    }
    return null;
  };
    ...
    battleChannel
      .on('postgres_changes', {
        ...
        setBattleState({
          ...
          startedAt: parseBattleDate(battle, 'started_at', 'start_time'),
          endsAt: parseBattleDate(battle, 'ends_at', 'end_time', 'ended_at'),
          suddenDeath: false,
          status: battle.status,
          scheduledStartAt: parseBattleDate(battle, 'scheduled_start_at', 'start_time'),
        });
      })
      .subscribe();
        ...
        if (currentBattle) {
          console.log('[BattleState] Found battle on mount:', currentBattle.id);
           setBattleState({
            ...
            startedAt: parseBattleDate(currentBattle, 'started_at', 'start_time'),
            endsAt: parseBattleDate(currentBattle, 'ends_at', 'end_time', 'ended_at'),
            suddenDeath: false,
            status: currentBattle.status,
            scheduledStartAt: parseBattleDate(currentBattle, 'scheduled_start_at', 'start_time'),
          });
      ...
      .on(
        ...
        (payload) => {
          ...
          setBattleState({
            ...
            startedAt: parseBattleDate(newBattle, 'started_at', 'start_time'),
            endsAt: parseBattleDate(newBattle, 'ends_at', 'end_time', 'ended_at'),
      ...
      .on(
        ...
        (payload) => {
          ...
          setBattleState(prev => ({
            ...
            endsAt: parseBattleDate(updated, 'ends_at', 'end_time', 'ended_at'),
          }));
      ...
      .on(
        ...
        (payload) => {
          ...
          setBattleState(prev => ({
            ...
            challengerScore: updated.challenger_score || 0,
            suddenDeath: updated.sudden_death || false,
            startedAt: parseBattleDate(updated, 'started_at', 'start_time') || prev.startedAt,
            endsAt: parseBattleDate(updated, 'ends_at', 'end_time', 'ended_at') || prev.endsAt,
            status: updated.status,
            hostReady: updated.host_ready || false,
            opponentReady: updated.opponent_ready || false,
            scheduledStartAt: parseBattleDate(updated, 'scheduled_start_at', 'start_time') || prev.scheduledStartAt,
          }));
