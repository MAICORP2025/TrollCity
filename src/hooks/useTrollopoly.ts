...
export function useTrollopoly({ streamId, isHost, enabled = true, seatUserIds = [] }: UseTrollopolyOptions) {
  ...
  useEffect(() => {
    ...
    channel.on('broadcast', { event: 'dice-roll' }, ({ payload }) => {
      ...
      if (typeof payload.dice === 'number') {
          const match = useTrollopolyStore.getState().match;
        if (match) {
    ...
    channel.on('broadcast', { event: 'match-end' }, ({ payload }) => {
      if (payload._senderId === user?.id) return;
      store.endGame(payload.winnerId || '');
    });
    ...
    channel.subscribe((status) => {
      if (import.meta.env.DEV) console.log('[useTrollopoly] Channel status:', status);
    });
  ...
  useEffect(() => {
    if (!enabled || !streamId || store.match) return;
    try {
      const raw = localStorage.getItem(`trollopoly-match-${streamId}`);
      if (raw) {
        store.setMatch(JSON.parse(raw));
      }
    } catch (err) {
      console.warn('[useTrollopoly] Failed to restore local match state:', err);
    }
  }, [enabled, streamId, store.match, store]);

  useEffect(() => {
    if (!enabled || !streamId) return;
    try {
      if (store.match?.streamId === streamId) {
        localStorage.setItem(`trollopoly-match-${streamId}`, JSON.stringify(store.match));
      } else if (!store.match) {
        localStorage.removeItem(`trollopoly-match-${streamId}`);
      }
    } catch (err) {
      console.warn('[useTrollopoly] Failed to persist local match state:', err);
    }
  }, [enabled, streamId, store.match]);
  ...
  useEffect(() => {
    if (!isHost || !store.match || store.match.phase !== 'lobby') return;
    void syncMatchState();
  }, [isHost, store.match?.countdownRemaining, store.match?.phase, syncMatchState]);

  useEffect(() => {
    return () => {
      if (!isHost || !streamId || originalBoxCountRef.current === null) return;
      supabase
        .from('streams')
        ...
        .then(({ error }) => {
          if (error) {
            console.error('[useTrollopoly] Failed to restore box_count on cleanup:', error);
          }
        });
    };
  }, [isHost, streamId]);
  ...
  const createGame = useCallback(async () => {
    ...
    const nextMatch = useTrollopolyStore.getState().match;
    // First broadcast simple "game created" event so viewers see it immediately
    await broadcastState('game-created', { matchId: nextMatch?.id });
    // Then broadcast full sync
    await broadcastState('match-sync', { match: nextMatch });
    toast.success('Trollopoly lobby created!');
  }, [isHost, user, profile, streamId, store, broadcastState]);
  ...
  const joinGame = useCallback(async () => {
    ...
    // Eligibility: broadcaster and users currently sitting in broadcast seats can join.
    if (!streamId) {
      toast.error('Stream not available');
      return;
    }

    // Fetch broadcaster ID for this stream
    ...
    if (streamError || !streamData) {
      toast.error('Failed to verify stream');
      return;
    }

    const broadcasterId = streamData.user_id;

    const isBroadcaster = user.id === broadcasterId;
    let isInSeat = seatUserIds.includes(user.id);
    if (!isInSeat) {
      const { data: activeSeat } = await supabase
        ...
        .eq('status', 'active')
        .maybeSingle();
      isInSeat = Boolean(activeSeat);
    }

    if (!isBroadcaster && !isInSeat) {
      toast.error('Join a broadcast seat first to play Trollopoly');
      return;
    }
  ...
  }, [user, profile, store, broadcastState, streamId, seatUserIds]);
  ...
  const selectPiece = useCallback(async (piece: TrollopolyPiece) => {
    if (!user || !store.match) return;
    if (!store.match.players.some((player) => player.id === user.id)) {
      await joinGame();
      const joinedMatch = useTrollopolyStore.getState().match;
      if (!joinedMatch?.players.some((player) => player.id === user.id)) {
        return;
      }
    }
  ...
  }, [user, store, joinGame, broadcastState]);

  const startGame = useCallback(async () => {
    if (!isHost || !store.match) return;
    if (store.match.players.length < DEFAULT_TROLLOPOLY_CONFIG.minPlayers) {
      toast.error(`Need at least ${DEFAULT_TROLLOPOLY_CONFIG.minPlayers} player to start`);
      return;
    }
    ...
    try {
      ...
      console.error('[useTrollopoly] Failed to capture original box_count:', err);
    }
  ...
  const rollDice = useCallback(async () => {
    ...
    const newPosition = (currentPlayer.position + dice) % store.match.properties.length;
    ...
    if (landedProperty && landedProperty.ownerId && landedProperty.ownerId !== user.id) {
      ...
      const realOwnerPayout = Math.floor(rent / 2);
      ...
      if (canAfford) {
        ...
        if (result.success) {
          if (realOwnerPayout > 0) {
            await addCoins({
              userId: landedProperty.ownerId,
              amount: realOwnerPayout,
              type: 'reward',
              description: `Trollopoly real rent share`,
              metadata: { fromPlayerId: user.id, propertyId: landedProperty.id, fullRent: rent },
            });
          }
          
          store.payRent(user.id, landedProperty.ownerId, rent);
          await syncMatchState();
          toast.success(`Paid ${rent} coins rent. ${realOwnerPayout} coins went to the owner.`);
        }
      } else {
        store.setMatch({
          ...store.match,
  ...
  const buyProperty = useCallback(async (propertyId: number) => {
    ...
    if (currentPlayer.balance < property.price) {
      toast.error('Not enough game coins to buy this property');
      return false;
    }
    ...
    const result = await deductCoins({
      ...
      metadata: { propertyId, propertyName: property.name, skipProfileRefresh: true },
    });
  ...
  const endTurn = useCallback(async () => {
    ...
    const activePlayer = store.match.players[store.match.currentTurnIndex];
    if (!isHost && activePlayer?.id !== user?.id) return;
    store.nextTurn();
  ...
  }, [isHost, store, broadcastState, user?.id]);
