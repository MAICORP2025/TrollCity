...
export function useParticipantAttributes(participantIds: string[], streamId: string) {
  ...
  useEffect(() => {
    ...
    // Build filter string for participant IDs
    // Note: Supabase realtime filters have limitations, so we construct OR chains for multiple IDs
    const participantFilter = participantIds.length > 0 
      ? participantIds.map(id => `user_id=eq.${id}`).join(',or')
      : '';

    // Subscribe to changes
    // 1. Gift changes (filtered by stream)
    ...
    // 2. Perk changes - filtered to participants only
    const perkChannel = supabase.channel(`attrs_perks:${participantIds.join('-')}`)
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
        table: 'user_perks',
        filter: participantFilter
         ...
         // Re-fetch attributes to update perk changes
         fetchData();
      })
      .subscribe();

    // 3. Profile changes - filtered to participants only
    const profileChannel = supabase.channel(`attrs_profiles:${participantIds.join('-')}`)
      .on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        table: 'user_profiles',
