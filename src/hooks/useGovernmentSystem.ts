...
export const useGovernmentSystem = () => {
  ...
  // Realtime subscription for law changes - ensures all users see updates instantly
  useEffect(() => {
    const lawChannel = supabase
      .channel('government_laws_changes')
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        () => {
          // Refetch laws when any change occurs (create, update, delete)
          fetchLaws();
        }
      )
      .subscribe();
    ...
    return () => {
      supabase.removeChannel(lawChannel);
    };
  }, [fetchLaws]);

  // Realtime subscription for vote changes - ensures vote counts update live
  useEffect(() => {
    const votesChannel = supabase
      .channel('law_votes_changes')
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        () => {
          // Refetch laws when any vote changes
          fetchLaws();
        }
      )
      .subscribe();
