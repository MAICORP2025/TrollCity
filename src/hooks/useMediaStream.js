...
export function useMediaStream() {
  ...
  useEffect(() => {
    const startStreamImmediately = async () => {
      try {
        ...
        // Guard: navigator.mediaDevices may be undefined on iOS/in-app browsers without permissions-policy
        if (typeof navigator === 'undefined' || !navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
          const msg = 'Camera/microphone access is not available in this browser or context. On iOS, ensure you are using Safari (not an in-app browser) and have granted camera permissions.';
          console.error('[useMediaStream]', msg);
          setError(msg);
          throw new Error(msg);
        }
  ...
  const startStream = useCallback(async (options = {}) => {
    try {
      ...
      if (typeof navigator === 'undefined' || !navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        const msg = 'Camera/microphone access is not available in this browser or context.';
        console.error('[useMediaStream]', msg);
        setError(msg);
        throw new Error(msg);
      }
