...
const MAX_ACTIVE_GIFT_ANIMATIONS = 5;
...
export const useAnimationStore = create<AnimationState>((set, get) => ({
  ...
  playGiftAnimation: (gift) => {
    ...
    set((state) => {
      const nextGifts = [...state.giftAnimations, { ...gift, id, timestamp }];
      if (nextGifts.length > MAX_ACTIVE_GIFT_ANIMATIONS) {
        // Drop oldest gift animations first to avoid browser overload
        return { giftAnimations: nextGifts.slice(nextGifts.length - MAX_ACTIVE_GIFT_ANIMATIONS) };
      }
      return { giftAnimations: nextGifts };
    });
