...
export function preloadRegistrySounds(): void {
  ...
  GIFT_ANIMATION_REGISTRY.forEach(e => {
    sounds.add(e.sound.includes('/sounds/lighter.mp3') ? '/sounds/click.mp3' : e.sound);
    sounds.add(e.fallbackSound.includes('/sounds/lighter.mp3') ? '/sounds/click.mp3' : e.fallbackSound);
  });
  sounds.forEach(src => {
