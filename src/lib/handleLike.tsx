...
function useLiveKitGamingViewer(stream: GamingStream | null, userId: string | undefined) {
  ...
  const leave = useCallback(async () => {
    try {
      ...
      console.warn('[GamingViewerPage] LiveKit leave failed:', error)
    } finally {
  ...
  const join = useCallback(async () => {
    ...
    try {
      ...
      room.on(RoomEvent.ParticipantConnected, (participant: RemoteParticipant) => {
        ...
        videoPubs.forEach((pub) => {
          if (pub.track && pub.track.kind === 'video') {
            ...
            if (videoContainerRef.current) {
              ...
              } catch (e) { console.warn('[GamingViewerPage] video attach failed:', e) }
            }
          }
        })
        audioPubs.forEach((pub) => {
          if (pub.track && pub.track.kind === 'audio') {
            ...
            } catch (e) { console.warn('[GamingViewerPage] audio attach failed:', e) }
          }
        })
      })
      ...
      room.on(RoomEvent.TrackSubscribed, (track: any, publication: any, participant: RemoteParticipant) => {
        if (!mountedRef.current) return
        if (track.kind === 'video') {
      ...
      room.on(RoomEvent.TrackUnsubscribed, (_track: any, _publication: any, _participant: RemoteParticipant) => {
        if (!mountedRef.current) return
      })
      ...
      console.error('[GamingViewerPage] LiveKit join failed:', error)
  ...
  useEffect(() => {
    ...
    if (stream?.livekit_room_name && userId) {
      join()
    }
...
function LiveKitVideoSurface({ track, containerRef }: { track: RemoteVideoTrack | null; containerRef: React.RefObject<HTMLDivElement | null> }) {
  useEffect(() => {
    ...
    try {
      ...
      console.warn('[GamingViewerPage] Failed to attach LiveKit video:', error)
    }
          ...
          <div className="absolute inset-0">
            {watchable && shouldUseLiveKit && liveKitViewer.remoteVideoTrack ? (
              ...
              <CenterStatus
                ...
                detail="Joining the live gaming stream via LiveKit."
              />
              ...
              <CenterStatus
                ...
                detail="Connected to LiveKit room. Waiting for the broadcaster to publish video."
              />
            ) : watchable ? (
              <CenterStatus
                icon={<MonitorPlay className="h-12 w-12 text-cyan-300" />}
