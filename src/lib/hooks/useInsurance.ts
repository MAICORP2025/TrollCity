...
import type { HomeownersInsurance, CarInsurance, BroadcastInsurance, InsuranceBrokerApplication } from '../../types/neighborhood'
...
export function useInsuranceBroker() {
  const { user } = useAuthStore()
  const [application, setApplication] = useState<InsuranceBrokerApplication | null>(null)
  const [loading, setLoading] = useState(true)
  ...
  const applyAsBroker = async (experience: string) => {
    if (!user?.id) return { success: false, error: 'Not authenticated' }
