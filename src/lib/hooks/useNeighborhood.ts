...
export function useNeighborhood() {
  ...
  const createNeighborhood = async (name: string, zipCode: string) => {
    ...
    try {
      ...
      // Create 15 empty houses for the neighborhood
      const housesData = []
      for (let i = 0; i < 15; i++) {
        housesData.push({
          neighborhood_id: data.id,
          owner_user_id: null, // Empty houses
          upgrade_level: 1,
          condition: 100,
          is_reposessed: false,
          electric_on: false,
      ...
      return { success: true, neighborhood: data, house }
    } catch (error: any) {
  ...
  const acceptInvite = async (leaderUserId: string) => {
    ...
    try {
      ...
      // Create free house for follower
