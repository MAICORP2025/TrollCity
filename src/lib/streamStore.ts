...
interface StreamState {
  ...
  setScreenAudioTrack: (track: any | null) => void;
...
export const useStreamStore = create<StreamState>()((set, get) => ({
  ...
  setScreenAudioTrack: (track) => {
    console.log('[StreamStore] Setting screen audio track:', track ? 'available' : 'null');
    set({ screenAudioTrack: track });
  },
  ...
  clearTracks: () => {
    ...
    const { screenTrack, screenAudioTrack, cameraTrack, screenPreviewStream } = get();
    ...
    if (screenAudioTrack && typeof screenAudioTrack.stop === 'function') {
      try {
        screenAudioTrack.stop();
      } catch (e) {
        console.warn('[StreamStore] Error stopping screen audio track:', e);
      }
    }
  ...
  initializeTracks: (params) => {
    ...
    set({
      ...
      screenAudioTrack: params.screenAudioTrack ?? get().screenAudioTrack,
