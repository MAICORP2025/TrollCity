...
export const useAuthStore = create<AuthState>()(
  persist(
    (set, get) => ({
      ...
      setProfile: (profile) => {
        ...
        // BUT allow if: it's first profile set (prevProfile is null), OR profile changed significantly
        const now = Date.now()
        const isFirstProfile = prevProfile === null || prevProfile === undefined
        const hasSignificantChange = prevProfile && profile && (
          prevProfile.id !== profile.id ||
          prevProfile.username !== profile.username ||
        ...
        if (!prevProfile && profile.username) {
          ...
          ]).then().catch(() => {}); // Suppress errors - doesn't matter if it fails
        }
  ...
  refreshProfile: async (force = false) => {
    ...
    // Set loading state and refreshing lock
    ...
    set({ isRefreshing: true })
        ...
        try {
          ...
          if (data) {
            ...
            // Clear loading state now that core profile is loaded
          ...
          get().setLoading(false)
            ...
            // Always release the lock and loading state
