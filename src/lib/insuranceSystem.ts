...
/**
 * Check if user's driver's license is currently suspended
 */
export async function isLicenseSuspended(userId: string): Promise<boolean> {
  try {
    ...
    console.error('Error checking license suspension:', err);
    return false;
  }
}

/**
 * Get license suspension info for display
 */
export async function getLicenseSuspensionInfo(userId: string): Promise<{ suspended: boolean; suspendedUntil?: string } | null> {
  try {
    const { data, error } = await supabase
      .from('user_driver_licenses')
      .select('status, suspended_until')
    ...
    if (data.status === 'suspended' && data.suspended_until) {
      const suspendedUntil = new Date(data.suspended_until);
      return {
        suspended: suspendedUntil > new Date(),
    ...
    console.error('Error getting license suspension info:', err);
    return null;
  }
}
    ...
    const flowResult = await runStandardPurchaseFlow({
      ...
      ensureOwnership: async (client) => {
        ...
        if (sameTypeInsurance) {
          ...
          try {
            currentExpiry = new Date(sameTypeInsurance.expires_at);
            if (isNaN(currentExpiry.getTime())) {
              throw new Error('Invalid date');
            }
          } catch {
            // Existing expiry is invalid, reset to now
            currentExpiry = now;
          }
          const expiresAt = new Date(currentExpiry.getTime() + plan.duration_hours * 60 * 60 * 1000);
