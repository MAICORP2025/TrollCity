...
const TrollerInsurance = () => {
  ...
  const purchaseInsurance = async (package_: InsurancePackage) => {
    ...
    try {
      ...
      // Calculate expiry time based on now
      const baseExpiry = new Date()
      baseExpiry.setHours(baseExpiry.getHours() + package_.duration_hours)

      let finalExpiry = baseExpiry
      // Extend existing insurance if active and expiry is valid
      if (activeInsurance && activeInsurance.expires_at) {
        try {
          ...
          // Invalid date, ignore and use baseExpiry
        }
      }
