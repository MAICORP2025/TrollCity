  ...
  return (
    <div className={`min-h-screen p-6 pb-24 ${trollCityTheme.backgrounds.primary} ${trollCityTheme.text.primary}`}>
      ...
      <div className="relative max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className={trollCityTheme.text.secondary}>History of all vehicle purchases and sales.</p>
            </div>
          </div>
          <button
        ...
        {/* Transaction List */}
        <div className={`${trollCityTheme.components.card} overflow-hidden`}>
          {isLoadingTransactions ? (
            <div className="p-8 text-center">
              <div className="w-8 h-8 border-2 border-purple-500 border-t-transparent rounded-full animate-spin mx-auto" />
              <p className={`mt-4 ${trollCityTheme.text.muted}`}>Loading transactions...</p>
            </div>
          ) : transactions.length === 0 ? (
            <div className="p-12 text-center">
              <History className="w-16 h-16 mx-auto mb-4 opacity-50" />
              <h3 className={`text-xl font-bold ${trollCityTheme.text.primary} mb-2`}>No Transactions Yet</h3>
              <p className={trollCityTheme.text.muted}>Your vehicle transaction history will appear here.</p>
              <button
                onClick={() => navigate('/ktauto')}
                className={`mt-4 px-6 py-3 ${trollCityTheme.gradients.button} rounded-lg font-bold transition hover:shadow-lg hover:-translate-y-0.5 inline-flex items-center gap-2`}
              >
