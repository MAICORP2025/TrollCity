...
import { useParams, useNavigate, useInRouterContext, useSearchParams } from 'react-router-dom';
           ...
           <div className="flex items-end relative">
             <img
               ...
               onError={(e) => {
                ...
                onClick={() => {
                  if (isProfileLive && liveStreamId) {
                    navigate(`/watch/${liveStreamId}`);
                  }
                }}
             />
              {isProfileLive && (
            ...
            {profile.license_plate && (
              <div className="md:ml-4 flex items-center gap-2 mt-2 md:mt-0">
                <div className="bg-gradient-to-r from-blue-600 to-blue-800 px-3 py-1 rounded-lg border border-blue-500/50 shadow-lg">
                  <span className="text-white font-mono font-bold text-sm tracking-wider">
                    {profile.license_plate}
                  </span>
                </div>
                <span className="text-xs text-gray-400 hidden md:inline">License Plate</span>
              </div>
            )}
                        ...
                        >
                          ...
                          <div className="flex flex-col items-end gap-1">
                            ...
                            {isOwnProfile && (
                              <button
                                type="button"
                                onClick={() => navigate(listing.listing_type === 'auction' ? '/auctions' : '/ktauto')}
                                className="px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-xs font-medium"
                              >
                                Manage Listing
                              </button>
