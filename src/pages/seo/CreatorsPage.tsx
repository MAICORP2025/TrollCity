...
const benefits = [
  { icon: DollarSign, title: 'Earn Money', description: 'Earn up to $4,000 a month from gifts, tips, and subscriptions' },
...
function CreatorStatsSection() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        // Get unique creators who broadcasted in last 30 days
        ...
        // Get total paid to creators from creator_earnings
        ...
        const topEarner = Math.max(...(earningsData?.map(r => r.total_earnings_usd || 0) || [0]))
  ...
  const statItems = [
    ...
    { value: loading ? '...' : formatCurrency(stats.topEarner), label: 'Top Monthly Earner' },
  ]
