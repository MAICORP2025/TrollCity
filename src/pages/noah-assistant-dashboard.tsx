...
const NoahAssistantDashboard = () => {
  ...
  const isNoahAssistant =
    ...
    profile?.troll_role === 'noah_assistant'
  const isAdmin =
  ...
  const isNoah = profile?.troll_role === 'noah'

  const canAccess = isNoahAssistant || isAdmin || isNoah
      ...
      <div className="min-h-screen bg-[#050507] px-4 py-10 text-white">
        <div className="mx-auto max-w-3xl rounded-[2rem] border border-red-400/30 bg-red-500/10 p-8 shadow-2xl shadow-red-950/30">
          <div className="flex items-center gap-3 text-red-100">
            ...
            <h1 className="text-2xl font-black">Noah Assistant Dashboard access required</h1>
          </div>
          <p className="mt-4 text-sm leading-7 text-red-100/90">
            Your current role is {profile?.role || 'unknown'}. This page is restricted to Noah Assistant, Admin, and Noah roles.
          </p>
  ...
  const loadDashboardStats = async () => {
    try {
      ...
      console.error('Failed to load Noah Assistant dashboard stats:', err)
  ...
  const forwardBatchToAdmin = async () => {
    ...
    try {
      const { data, error } = await supabase.rpc('assistant_forward_payout_batch', {
        ...
        p_assistant_username: profile?.username || 'noah_assistant',
    ...
    <div className="min-h-screen bg-[#050507] text-white">
      ...
      <main className="relative mx-auto flex max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
          ...
          <div className="max-w-3xl">
           ...
           <div className="flex gap-3">
             <button
               ...
               disabled={loading}
               className={`rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-sm font-black text-white transition-all hover:bg-white/20 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
             >
        ...
        {activeSection === 'user_reports' && (
          <div className="rounded-xl border border-white/10 bg-white/[0.04] p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setActiveSection('overview')} className="rounded-xl border border-white/10 bg-white/10 px-4 py-2 text-sm font-black text-white hover:bg-white/20 flex items-center gap-2">
              ...
              <div className="space-y-3">
                {userReportsList.map((report) => (
                  <div key={report.id} className="rounded-lg border border-white/10 bg-black/20 overflow-hidden">
                    <div className="p-4 flex justify-between items-center cursor-pointer hover:bg-white/5" onClick={() => setExpandedReport(expandedReport === report.id ? null : report.id)}>
                      <div>
                        <p className="font-black text-white text-sm">{report.reason || 'User Report'}</p>
                        <p className="text-xs text-slate-400 mt-1">Reporter: {report.reporter?.display_name || report.reporter?.username || 'Unknown'} → Reported: {report.reported_user?.display_name || report.reported_user?.username || 'Unknown'} • {new Date(report.created_at).toLocaleDateString()}</p>
                      </div>
              ...
              <div className="space-y-3">
                {moderationActionsList.map((action) => (
                  <div key={action.id} className="rounded-lg border border-white/10 bg-black/20 overflow-hidden">
                    <div className="p-4 flex justify-between items-center cursor-pointer hover:bg-white/5" onClick={() => setExpandedModAction(expandedModAction === action.id ? null : action.id)}>
                      <div>
                        <p className="font-black text-white text-sm">{action.action_type || 'Moderation Action'}</p>
                        <p className="text-xs text-slate-400 mt-1">Actor: {action.actor?.display_name || action.actor?.username || 'Unknown'} → Target: {action.target_user?.display_name || action.target_user?.username || 'Unknown'} • {new Date(action.created_at).toLocaleDateString()}</p>
                      </div>
                      <div className="flex items-center gap-2">
              ...
              <div className="space-y-3">
                {applicationsList.map((app) => (
                  <div key={app.id} className="rounded-lg border border-white/10 bg-black/20 overflow-hidden">
                    <div className="p-4 flex justify-between items-center cursor-pointer hover:bg-white/5" onClick={() => setExpandedApplication(expandedApplication === app.id ? null : app.id)}>
                      <div>
                        <p className="font-black text-white text-sm">Role: {app.role_requested || 'Unknown Role'}</p>
                        <p className="text-xs text-slate-400 mt-1">Applicant: {app.user_profiles?.display_name || app.user_profiles?.username || 'Unknown'} • {new Date(app.created_at).toLocaleDateString()}</p>
                      </div>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400">{log.user_profiles?.display_name || 'Unknown User'} • {new Date(log.created_at).toLocaleTimeString()}</p>
                        </div>
                      </div>
                      <div className="text-xs font-black text-slate-400">{new Date(log.created_at).toLocaleDateString()}</div>
                    </div>
                    {log.details && (
                      <div className="mt-2 p-3 rounded bg-white/5 text-xs text-slate-400">
        ...
        {activeSection === 'staff_reports' && (
          <div className="rounded-xl border border-white/10 bg-white/[0.04] p-6 mt-6">
            <div className="flex items-center justify-between mb-6">
            ...
            <ExecutiveReportsList viewMode="noah_assistant" />
          </div>
        )}
...
export default NoahAssistantDashboard