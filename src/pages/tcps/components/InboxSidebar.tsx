...
const isMissingDisplayName = (value?: string | null) => {
  const normalized = (value || '').trim().toLowerCase()
  return !normalized || normalized === 'unknown' || normalized === 'unknown user'
}

const fallbackUsername = (userId?: string | null) => userId ? `user${userId.slice(0, 6)}` : 'Troll Citizen'
...
export default function InboxSidebar({
  ...
  const blockedIdsLastRef = useRef(0)
  ...
  const fetchConversations = useCallback(async (isBackground = false) => {
    ...
    try {
      ...
      let allConvs: SidebarConversation[] = (rpcData || []).map((c: any) => {
        const username = isMissingDisplayName(c.other_username)
          ? (c.other_display_name || c.display_name || c.username || fallbackUsername(c.other_user_id))
          : c.other_username
        return {
      ...
      const missingProfileIds = allConvs
        .filter((conv) => !conv.is_group && isMissingDisplayName(conv.other_username))
      ...
      if (missingProfileIds.length > 0) {
        ...
        allConvs = allConvs.map((conv) => {
          if (!isMissingDisplayName(conv.other_username)) return conv
          const profile = profileMap.get(conv.other_user_id) as any
          if (!profile) return { ...conv, other_username: fallbackUsername(conv.other_user_id) }
          return {
            ...conv,
            other_username: profile.display_name || profile.username || fallbackUsername(conv.other_user_id),
            other_avatar_url: conv.other_avatar_url || profile.avatar_url || null,
            other_created_at: conv.other_created_at || profile.created_at,
            rgb_username_expires_at: conv.rgb_username_expires_at || profile.rgb_username_expires_at,
            glowing_username_color: conv.glowing_username_color || profile.glowing_username_color,
          }
        })
      }
      ...
      const now = Date.now()
      const cacheAge = now - blockedIdsLastRef.current
      const shouldRefreshBlocks = cacheAge > 5 * 60 * 1000 || blockedIdsRef.current.size === 0
      if (shouldRefreshBlocks) {
        const blockedIds = await getBlockedUserIds()
        blockedIdsRef.current = new Set(blockedIds)
        blockedIdsLastRef.current = now
      }
  ...
  useEffect(() => {
    ...
    const canFilterByConvIds = convIds.length > 0 && convIds.length <= 100
    const conversationFilter = canFilterByConvIds ? `conversation_id=in.(${convIds.join(',')})` : undefined

    const messagesChannel = supabase.channel('sidebar-messages')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'conversation_messages', filter: `sender_id=eq.${user?.id}` }, (_payload) => {
        fetchConversations(true)
      })

    if (conversationFilter) {
      messagesChannel.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'conversation_messages', filter: conversationFilter }, (payload) => {
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'conversation_messages', filter: conversationFilter }, () => {
        debouncedFetch()
      })
    }

    messagesChannel.on('broadcast', { event: 'new-message' }, () => {
      debouncedFetch()
    })
    .subscribe()
  ...
  }, [user?.id, isUserOfficer, activeConversation, conversationIds.join(',')])
                        ...
                        {isGroup ? (
                          ...
                          <UserNameWithAge 
                            user={{
                              ...
                              rgb_username_expires_at: conv.rgb_username_expires_at,
