...
export default function InboxSidebar({
  ...
  const fetchConversations = useCallback(async (isBackground = false) => {
    ...
    try {
      ...
      const allConvs: SidebarConversation[] = (rpcData || []).map((c: any) => {
        const username = c.other_username || `user${c.other_user_id?.slice(0, 6)}`
        return {
  ...
  useEffect(() => {
    ...
    const messagesChannel = supabase.channel('sidebar-messages')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'tcps_messages', filter: `sender_id=eq.${user?.id}` }, (_payload) => {
        fetchConversations(true)
      })

    if (conversationFilter) {
      messagesChannel.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'tcps_messages', filter: conversationFilter }, (payload) => {
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'tcps_messages', filter: conversationFilter }, () => {
        debouncedFetch()
      })
    }
