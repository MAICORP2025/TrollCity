    ...
    <div className="max-w-2xl mx-auto p-4">
      ...
      <div className="bg-slate-900/80 rounded-2xl p-6 border border-slate-800">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Upload Image *</label>
            <input
              type="file"
              accept="image/*"
              onChange={(e) => {
              ...
              className="w-full bg-zinc-900 border border-white/15 rounded-xl px-3 py-2.5 text-sm text-white file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-purple-600 file:text-white hover:file:bg-purple-500"
              required
            />
            {selectedImage && (
          ...
          <button
            type="submit"
            disabled={loading || uploadingImage || !profile || profile.troll_coins < 1000 || !formData.title || !selectedImage}
          ...
          >
            {loading || uploadingImage ? 'Submitting...' : 'Submit Advertisement'}
          </button>
        </form>
      </div>
    </div>
