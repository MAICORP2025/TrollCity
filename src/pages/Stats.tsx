...
export default function Stats() {
  ...
  }, [user?.id, level, xpTotal, xpToNext, profile?.role, profile?.troll_role, balances.troll_coins, balances.paid_coins, balances.cashout_coins, balances.cashout_reserved_coins])
            ...
            <div className="bg-[#1A1A24] border border-green-500/30 rounded-2xl p-6 hover:border-green-500/60 transition-colors">
              ...
              <div className="space-y-3">
                ...
                {(stats.cashout_coins > 0 || stats.cashout_reserved_coins > 0) && (
                  <div className="bg-[#1A1A24] border border-emerald-500/30 rounded-lg p-4 mt-3">
                    <div className="flex justify-between items-center mb-2">
                    ...
                    <div className="text-xs text-emerald-500 space-y-1">
                      {stats.cashout_reserved_coins > 0 && (
                        <div>{stats.cashout_reserved_coins.toLocaleString()} coins reserved for Friday</div>
                      )}
                      {stats.cashout_coins > 0 && stats.cashout_coins <= stats.troll_coins + stats.paid_coins && (
                        <div>{stats.cashout_coins.toLocaleString()} coins available (min {stats.cashout_coins >= 5000 ? 'met' : 'need ' + (5000 - stats.cashout_coins) + ' more'})</div>
                      )}
                    </div>
                    {stats.cashout_coins + stats.cashout_reserved_coins > stats.troll_coins + stats.paid_coins && (
                      <div className="mt-2 pt-2 border-t border-amber-500/30 text-xs text-amber-400">
                        ⚠️ Some cashout from past deposits exceeds current balance
                      </div>
                    )}
                  </div>
                )}
