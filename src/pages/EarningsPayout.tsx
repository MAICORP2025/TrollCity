...
export default function EarningsPayout() {
  ...
  const troll_coins = Math.max(0, earned_coins - reserved_coins)
  
  const freeCoins = (profile as any)?.free_coins || 0
  ...
  useEffect(() => {
    ...
    const loadActiveLoanState = async () => {
      if (!profile?.id) {
      ...
      const { data, error } = await supabase
        ...
        .select('id')
        .eq('user_id', profile.id)
        .in('status', ['active', 'late', 'overdue', 'delinquent', 'defaulted'])
        .limit(1)

      if (cancelled) return

      if (error) {
        console.warn('Could not check active loans before cashout:', error.message)
        setHasActiveLoans(false)
        return
      }

      setHasActiveLoans((data || []).length > 0)
    }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (hasActiveLoans) {
      toast.error('Active loans must be paid before cashout.')
      return
    }
  ...
  }, [profile, eligibleTiers.length, selectedTierId, troll_coins, hasActiveLoans, payoutDetails, fullName, loadRecent])
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div className="bg-[#17131F] rounded-lg p-4 border border-indigo-500/20">
              ...
              <p className="text-lg font-semibold">
                {hasActiveLoans ? 'Loan hold' : eligibleTiers.length ? `${eligibleTiers.length} tier(s)` : 'None yet'}
              </p>
            </div>
          </div>
        </div>
  ...
  {CASHOUT_TIERS.map(tier => (
    <option key={tier.id} value={tier.id} disabled={tier.coins > troll_coins}>
      {tier.label} {tier.coins > troll_coins ? '(not eligible)' : ''}
          ...
          <button
            disabled={loading || hasActiveLoans}
