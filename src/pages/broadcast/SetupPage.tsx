...
import { BROADCAST_THEMES, getBroadcastTheme } from '../../lib/broadcastThemes';
import {
...
export default function SetupPage() {
  ...
  const [selectedTheme, setSelectedTheme] = useState<string>(() => {
    // Default to CEO theme for CEO users
    const isCEO = profile?.username === 'ceo' || profile?.role === 'ceo' || profile?.role === 'admin' || profile?.role === 'superadmin' || profile?.role === 'owner';
    if (isCEO) {
      return 'ceo_gold_premium';
    }
    return 'default';
  });
  ...
  // Load owned themes and user preferences
  useEffect(() => {
    if (user?.id) {
      // Load purchased themes and user preferences in parallel
      Promise.all([
        supabase
      ...
      .then(async ([{ data: purchaseData }, { data: profileData }]) => {
        let themes = purchaseData ? purchaseData.map(row => row.theme_id) : [];

        if (themes.length > 0) {
          // Get theme slugs for the purchased UUIDs
          const themeUUIDs = themes;
          const { data: themeData } = await supabase
            .from('broadcast_background_themes')
            .select('id, slug')
            .in('id', themeUUIDs);

          themes = themeData ? themeData.map(row => row.slug) : [];
        }

        // CEO users get access to CEO theme automatically
        const isCEO = profile?.username === 'ceo' || profile?.role === 'ceo' || profile?.role === 'admin' || profile?.role === 'superadmin' || profile?.role === 'owner';
        if (isCEO && !themes.includes('ceo_gold_premium')) {
          themes.push('ceo_gold_premium');
        }

        setOwnedThemes(themes);

        // Set user's selected theme if they have one saved
        if (profileData?.selected_theme_slug && themes.includes(profileData.selected_theme_slug)) {
          setSelectedTheme(profileData.selected_theme_slug);
        } else if (isCEO) {
          setSelectedTheme('ceo_gold_premium');
        }
      })
      .catch(err => console.error('Error loading themes and preferences:', err));
    }
  }, [user?.id, profile]);
  ...
  // Battle format state
  const [battleFormat, setBattleFormat] = useState<'1v1' | '2v2' | '3v3' | '4v4' | '5v5'>('3v3');
  ...
  }, [screenTrack, streamMode, setStreamMode]);
  ...
  const handleStartStream = async () => {
    ...
    try {
      ...
      // Theme Selection - CEO gets forced theme, others can choose
      const isCEOUser = profile?.username === 'ceo' || profile?.role === 'ceo' || profile?.role === 'admin' || profile?.role === 'superadmin' || profile?.role === 'owner';
      let selectedThemeUrl = null;
      let layoutMode = categoryConfig.layoutMode === 'debate' ? 'split' :
                       categoryConfig.layoutMode === 'classroom' ? 'grid' :
                       categoryConfig.layoutMode === 'spotlight' ? 'spotlight' : 'grid';

      if (isCEOUser) {
        // Force CEO theme and cinematic layout
        selectedThemeUrl = 'ceo_gold_premium';
        layoutMode = 'cinematic_stage';
      } else if (selectedTheme !== 'default' && ownedThemes.includes(selectedTheme)) {
        // User selected a theme
        selectedThemeUrl = selectedTheme; // Use the theme slug directly
      }
      ...
      const insertData: Record<string, unknown> = {
        ...
        broadcast_theme_slug: selectedTheme,
      ...
      // Stream created with status 'live' - no background update needed
      ...
      PreflightStore.setLivekitTracks(livekitTracksRef.current);
        ...
        >
          ...
          {ownedThemes.map((themeSlug) => (
            <option key={themeSlug} value={themeSlug}>
              {themeSlug.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
            </option>
          ))}
        </select>
      </div>
      ...
      case 'battle':
        return (
          <div className="flex flex-col gap-2">
            <span className="text-[10px] font-bold uppercase tracking-wider text-red-400">Universal Battle</span>
            <p className="text-xs text-white">Team-based battles — viewers gift to Side A or B</p>
            <div className="flex flex-col gap-1 mt-1">
              <span className="text-[9px] text-gray-400 uppercase">Select Battle Format:</span>
