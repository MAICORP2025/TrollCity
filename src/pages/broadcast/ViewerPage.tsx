...
import { RemoteParticipant, RemoteAudioTrack, RemoteVideoTrack } from 'livekit-client'
...
// Simple AudioPlayer component for playing remote audio tracks
function AudioPlayer({ audioTrack }: { audioTrack: RemoteAudioTrack }) {
  ...
  useEffect(() => {
    if (audioTrack && !audioRef.current) {
      try {
        ...
        console.log('[AudioPlayer] Attached audio track');
      } catch (err) {
        console.error('[AudioPlayer] Failed to attach audio:', err);
      }
    }
...
function ViewerPage() {
  ...
  // Get broadcast theme for viewer
  const viewerBroadcastTheme = getBroadcastTheme(stream?.broadcast_theme_slug, stream?.category || '');
    ...
    const initLiveKit = async () => {
      try {
        ...
        if (user?.id) {
          ...
          console.log('[ViewerPage] Remote participants:', existingParticipants.map(p => ({
            identity: p.identity,
            hasVideo: p.isCameraEnabled,
            hasAudio: p.isMicrophoneEnabled,
            tracks: Array.from((p as any).trackPublications?.values?.() || []).map((t: any) => ({
              trackSid: t.trackSid,
              name: t.trackName,
              kind: t.kind,
          ...
          if (stream?.user_id && existingParticipants.length > 0) {
            // Map all participants to their identities for gift targeting
            const participantMap: Record<string, RemoteParticipant> = {};
            existingParticipants.forEach(p => {
              // Try to extract user ID from identity (format: "userId-username" or just userId)
              const identityParts = p.identity.split('-');
              const potentialUserId = identityParts[0];
              if (potentialUserId && potentialUserId.length > 0) {
                participantMap[potentialUserId] = p;
              }
              // Also map by full identity for fallback
              participantMap[p.identity] = p;
            });
            setUserIdToParticipant(participantMap);
          }
  ...
  const getParticipantVideoTrack = (participant: RemoteParticipant) => {
    console.log('[ViewerPage] Getting video track for:', participant.identity);
    console.log('[ViewerPage] Video publications:', Array.from(participant.videoTrackPublications.values()).map(p => ({
      name: p.trackName,
      kind: p.kind,
    ...
    // First try to find screen share track
    const screenSharePub = Array.from(participant.videoTrackPublications.values()).find(p => 
      p.isSubscribed && p.track && p.trackName === 'screen-share'
    );
    if (screenSharePub?.track) {
      console.log('[ViewerPage] Found screen share track!');
      return screenSharePub.track;
    }
    
    // Then any video track
...
<BattleView
              ...
              <div className={cn(
                ...
                viewerBroadcastTheme?.shellClassName
              )}>
                {/* Theme overlay */}
                {viewerBroadcastTheme && <div className={cn('absolute inset-0 pointer-events-none', viewerBroadcastTheme.overlayClassName)} />}
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          viewerBroadcastTheme?.playerFrameClassName
                        )}
                        onClick={(e) => {
                          e.stopPropagation();
                          e.preventDefault();
                          // Try to extract user ID from participant identity
                          const identityParts = participant.identity.split('-');
                          const participantUserId = identityParts[0] || participant.identity;
                          if (participantUserId) {
                            onGift(participantUserId);
                          } else if (stream?.user_id) {
                            onGift(stream.user_id);
                          }
                        }}
                      >
                        {videoTrack ? (
