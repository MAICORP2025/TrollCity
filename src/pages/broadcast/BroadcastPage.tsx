...
import ViewerBubbles from '../../components/broadcast/ViewerBubbles'
...
import TrollopolyCityDevTest from '../../pages/dev/TrollopolyCityDevTest'
...
import ArrestHandler from '../../components/broadcast/ArrestHandler'
import KickHandler from '../../components/broadcast/KickHandler'
  ...
  const fetchHostStreamFallback = async () => {
    ...
    try {
      const { data: fallbackData, error: fallbackError } = await supabase
        .from('streams')
        .select('*, total_likes, hls_url, is_battle, battle_id, battle_mode, battle_format, battle_status, side_a_score, side_b_score')
        .eq('user_id', user.id)
        .maybeSingle()
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        const { data, error } = await supabase
          .from('streams')
          .select('hls_url, status, box_count, is_battle, battle_id, has_rgb_effect, are_seats_locked, total_likes, seat_price, current_viewers, total_gifts_coins, battle_mode, battle_format, battle_status, side_a_score, side_b_score')
          .eq('id', streamId)
          .maybeSingle()
          ...
          try {
            ...
            setStream((prev: any) => {
              ...
              return {
                ...
                battle_mode: payload.new.battle_mode,
                battle_format: payload.new.battle_format,
                battle_status: payload.new.battle_status,
                side_a_score: payload.new.side_a_score,
                side_b_score: payload.new.side_b_score,
              };
            });
...
return
        ...
        // No existing room - try to publish preflight tracks or cached camera track before falling back to camera/mic enablement
        const publishAudioTrack = preflightTracks?.[0]
        const publishVideoTrack = !isScreenShareExisting ? (preflightTracks?.[1] || cameraTrack) : undefined
        const hasPublishableTrack = !!publishAudioTrack || !!publishVideoTrack || (isScreenShareExisting && screenTrackExisting)

        if (hasPublishableTrack) {
          console.log('[BroadcastPage] Publishing startup tracks to room', {
            hasAudio: !!publishAudioTrack,
            hasVideo: !!publishVideoTrack,
            isScreenShare: isScreenShareExisting,
            hasScreenTrack: !!screenTrackExisting
          })

          let publishedAudio: LocalAudioTrack | undefined
          let publishedVideo: LocalVideoTrack | undefined

          if (publishAudioTrack) {
            publishedAudio = await publishTrackOrClone(publishAudioTrack, room, 'audio')
            if (publishedAudio) {
              console.log('[BroadcastPage] Published preflight audio track')
            }
          }

          if (publishVideoTrack) {
            publishedVideo = await publishTrackOrClone(publishVideoTrack, room, 'video')
            if (publishedVideo) {
              console.log('[BroadcastPage] Published preflight video track')
            }
          }

          if (publishedAudio && preflightEnabledStates.isAudioEnabled) {
            try {
              const mediaTrack = publishedAudio.mediaStreamTrack
              if (mediaTrack) mediaTrack.enabled = true
              if (typeof publishedAudio.enable === 'function') publishedAudio.enable()
            } catch (err) {
              console.warn('[BroadcastPage] Error enabling preflight audio track:', err)
            }
          }

          if (publishedVideo && preflightEnabledStates.isVideoEnabled) {
            try {
              const mediaTrack = publishedVideo.mediaStreamTrack
              if (mediaTrack) mediaTrack.enabled = true
              if (typeof publishedVideo.enable === 'function') publishedVideo.enable()
            } catch (err) {
              console.warn('[BroadcastPage] Error enabling preflight video track:', err)
            }
          }

          if (publishedAudio || publishedVideo) {
            setLocalTracks([publishedAudio || null, publishedVideo || null])
          }

          let publishedSomething = !!publishedAudio || !!publishedVideo

          if (isScreenShareExisting && screenTrackExisting) {
            try {
              await room.localParticipant.publishTrack(screenTrackExisting)
              setLocalTracks([publishedAudio || null, screenTrackExisting])
              setIsScreenSharing(true)
              publishedSomething = true
              console.log('[BroadcastPage] Published screen track for gaming mode')
            } catch (err) {
              console.error('[BroadcastPage] Failed to publish existing screen track:', err)
            }
          }
        ...
        console.log('[BroadcastPage] No PreflightStore room - enabling camera/mic directly')
        ...
        try {
          ...
          console.log('[BroadcastPage] enableCameraAndMicrophone succeeded')
          tracksCreated = true
        } catch (err) {
          console.error('[BroadcastPage] enableCameraAndMicrophone failed:', err)
        }
        
        // Get the published tracks from the room
        const publishedVideoTracks = Array.from(room.localParticipant.videoTrackPublications.values())
        const publishedAudioTracks = Array.from(room.localParticipant.audioTrackPublications.values())
        
        videoTrack = publishedVideoTracks.find(p => p.track)?.track as LocalVideoTrack | undefined
        audioTrack = publishedAudioTracks.find(p => p.track)?.track as LocalAudioTrack | undefined
        
        console.log('[BroadcastPage] Published tracks:', {
          hasVideo: !!videoTrack,
          hasAudio: !!audioTrack,
        ...
        // Only create new tracks if enableCameraAndMicrophone didn't create them
        if (!tracksCreated || (!videoTrack && !audioTrack)) {
          console.log('[BroadcastPage] No tracks from enableCameraAndMicrophone - creating manually')
...
for (const pub of room.localParticipant?.audioTrackPublications?.values?.() || []) {
          ...
          if (!audioTrack && !videoTrack) {
            const fallbackVideo = Array.from(room.localParticipant.videoTrackPublications.values()).find((pub) => pub.track)?.track as LocalVideoTrack | undefined
            const fallbackAudio = Array.from(room.localParticipant.audioTrackPublications.values()).find((pub) => pub.track)?.track as LocalAudioTrack | undefined
            if (fallbackAudio || fallbackVideo) {
              console.log('[BroadcastPage] Fallback local tracks from published participant:', {
                hasAudio: !!fallbackAudio,
                hasVideo: !!fallbackVideo,
                videoTrackId: fallbackVideo?.getTrackId?.(),
                audioTrackId: fallbackAudio?.getTrackId?.()
              })
              audioTrack = fallbackAudio
              videoTrack = fallbackVideo
            }
          }
        } else {
        ...
        // Check both PreflightStore AND sessionStorage for screen share mode to handle edge cases
        const isScreenShareLocal = !!(PreflightStore.getScreenShareMode() || sessionStorage.getItem('tc_broadcast_screen_mode') === 'true')
        ...
        if (!isScreenShareMode && !room.localParticipant.isCameraEnabled && (videoTrack || !tracksCreated)) {
        ...
        // Also check sessionStorage as fallback for screen share mode
        const isScreenShareForPublish = PreflightStore.getScreenShareMode() || sessionStorage.getItem('tc_broadcast_screen_mode') === 'true'
        if (isScreenShareForPublish) {
          ...
          // Also publish screen audio track if available
          const screenAudioTrackToPublish = screenAudioTrack
          if (screenAudioTrackToPublish) {
            console.log('[BroadcastPage] Gaming screen share - publishing screen audio track')
            try {
              await room.localParticipant.publishTrack(screenAudioTrackToPublish)
              console.log('[BroadcastPage] Screen audio track published successfully')
            } catch (err) {
              console.error('[BroadcastPage] Failed to publish screen audio track:', err)
            }
          }
        }
      ...
      <StreamLayout
        ...
        controls={
          <BroadcastControls
            ...
            isMicOn={roomRef.current?.localParticipant?.isMicrophoneEnabled ?? false}
            isCamOn={roomRef.current?.localParticipant?.isCameraEnabled ?? false}
            boxCount={boxCount}
            setBoxCount={updateBoxCount}
            onRefreshStream={refreshStream}
            fiveVFiveBattleActive={stream.is_battle}
            onFiveVFiveBattle={isHost ? handleStartBattle : undefined}
        ...
        overlays={
          <>
            {/* Mod Action Handlers - Monitor for arrests and kicks */}
            <ArrestHandler streamId={streamId || ''} />
            <KickHandler streamId={streamId || ''} />
            ...
            <AnimatePresence>
              {trollopoly.match && trollopoly.match.phase === 'playing' && (
                <div className="fixed inset-0 z-40 bg-black">
                  {/* Use dev preview version */}
                  <TrollopolyCityDevTest
                    videoTracks={getVideoTracksForParticipants()}
                    participants={getRemoteParticipantsArray()}
                    gameBalance={trollopoly.match?.gameBalance || 0}
                    playerBalances={trollopoly.match?.players?.reduce((acc, player) => {
                      acc[player.id] = player.balance;
                      return acc;
                    }, {} as { [userId: string]: number }) || {}}
                    onAddCoinsToGame={trollopoly.addCoinsToGame}
                    broadcasterId={stream?.user_id}
                    players={trollopoly.match?.players || []}
                  />
                </div>
              )}
            </AnimatePresence>
