...
/** Matches AdminDashboard/Economy: coin-pack and fiat-linked rows in coin_transactions */
...
export default function PaymentsDashboard() {
  ...
  // Check admin access (parity with AdminDashboard finance surfaces)
  useEffect(() => {
    if (!profile) return
    const allowed =
      profile.is_admin === true ||
      ['admin', 'superadmin', 'ceo', 'troll_officer'].includes(profile.role ?? '')
    if (!allowed) {
  ...
  useEffect(() => {
    loadTransactions()
    const refreshTimer = window.setInterval(loadTransactions, 60000)
  ...
  const loadTransactions = async () => {
    try {
      ...
      const transformed = (data || []).map((tx: any) => {
        const m = tx.metadata || {}
        const usdPaid = Number(tx.usd_amount || tx.platform_profit || m.amount_paid || m.amount_usd || m.usd_amount || m.total || m.amount || 0)
        const paymentRef =
          tx.paypal_order_id ||
        ...
        return {
          ...
          amount_paid: usdPaid,
          payment_id: typeof paymentRef === 'string' ? paymentRef : null,
