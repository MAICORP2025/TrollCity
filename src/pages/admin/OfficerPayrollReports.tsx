...
export default function OfficerPayrollReports() {
   ...
   const loadReports = async () => {
     ...
     try {
       ...
       console.error('Failed to load weekly role perk reports:', error)
       toast.error('Failed to load weekly role perk reports')
     } finally {
       setLoading(false)
     }
   }
       ...
       <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 text-white">
         <div className="max-w-6xl mx-auto px-6 py-10 space-y-6">
           <div className="flex items-center justify-between">
             <div>
               <h1 className="text-3xl font-black tracking-tight">Weekly Role Perks</h1>
               <p className="text-sm text-slate-300">Treasury weekly role perk payments and frozen officer flags</p>
             </div>
             <button
               onClick={loadReports}
               disabled={loading}
           ...
           <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
             ...
             <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
               <p className="text-xs uppercase text-slate-400">Perk Logs</p>
           ...
           <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
             <div className="flex items-center gap-2 mb-4">
               ...
               <h2 className="text-lg font-bold">Weekly Role Perk Logs</h2>
             </div>
             {loading ? (
               <div className="text-sm text-slate-400">Loading perk logs...</div>
             ) : logs.length === 0 ? (
               <div className="text-sm text-slate-400">No perk logs for the last 7 days.</div>
             ) : (
