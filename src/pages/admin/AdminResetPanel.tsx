...
export default function AdminResetPanel() {
  ...
  const handleReset = async (action: string, actionName: string) => {
    ...
    // Handle reset_troll_coins with direct RPC call
    if (action === 'reset_troll_coins') {
      try {
        const { error } = await supabase.rpc("reset_troll_coins", { p_user_id: profile.id, p_new_balance: 0 })
      ...
      <div className="space-y-4">
        ...
        <button
          ...
          onClick={(e) => {
            e.preventDefault()
            e.stopPropagation()
            handleReset('reset_troll_coins', 'Reset Coin Balances')
          }}
          disabled={loading !== null || confirmText !== 'RESET'}
