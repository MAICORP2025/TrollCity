...
export default function UserCompliancePrompt() {
  ...
  const completionPercentage = Math.round(((1 - missingItems.length) / 1) * 100);
        ...
        <div className="flex items-center gap-3 text-center sm:text-left">
          ...
          <div>
            <p className="font-bold text-sm flex items-center justify-center sm:justify-start gap-2">
              ...
              Action Required ({completionPercentage}% Complete - {1 - missingItems.length}/1 Step)
            </p>
            <p className="text-xs text-cyan-100">
              Please complete your Profile (Full Name) to ensure full account access.
            </p>
          </div>
        </div>
