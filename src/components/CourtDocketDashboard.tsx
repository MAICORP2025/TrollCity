...
const CourtDocketDashboard: React.FC = (): JSX.Element => {
  ...
  const hardDeleteDocket = async (docketId: string) => {
    if (!confirm('Permanently delete this docket entry? This cannot be undone.')) return;
    try {
      const { data, error } = await supabase.rpc('hard_delete_docket_entry', { p_docket_id: docketId });
      if (error || (data && data.success === false)) throw new Error(data?.error || error?.message || 'Delete failed');
      setDocketEntries(prev => prev.filter(entry => entry.id !== docketId));
    } catch (error) {
      console.error('Error deleting docket entry:', error);
    }
  };
                  ...
                  <div className="flex gap-1">
                    {canManageCases && (entry.status === 'scheduled' || entry.status === 'in_session') && (
                      <>
                        ...
                        <button
                          onClick={() => hardDeleteDocket(entry.id)}
