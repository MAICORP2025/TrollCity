...
export const getThreads = async (userId: string, folder: MailFolder = 'inbox'): Promise<UtromailThread[]> => {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      *,
      ...
      utromail_messages(id, body, sender_id, sent_at, sender_mail_address, user_profiles!utromail_messages_sender_id_fkey(username, display_name, avatar_url))
    `)
    .in('id', threadIds)
  ...
  // Collect all unique member user IDs across all threads to batch-fetch profiles
  const allMemberUserIds = new Set<string>();
  for (const t of data || []) {
    for (const m of t.utromail_thread_members || []) {
  ...
  if (allMemberUserIds.size > 0) {
    const { data: profiles } = await supabase
      ...
      .in('id', Array.from(allMemberUserIds));
    for (const p of profiles || []) {
      profileMap[p.id] = p;
    }
  }

  // Determine the user's folder for each thread and filter
  return (data || [])
    .filter((t: any) => {
      const myMember = (t.utromail_thread_members || []).find((m: any) => m.user_id === userId);
      if (!myMember) return false;
      if (folder === 'inbox') return myMember.folder === 'inbox' || myMember.folder === 'sent';
      if (folder === 'sent') return myMember.folder === 'sent';
      return myMember.folder === folder;
    })
    .map((t: any) => {
      // Attach user profile data to each member
      const membersWithProfiles = (t.utromail_thread_members || []).map((m: any) => ({
        ...m,
        ...(profileMap[m.user_id] || {}),
      }));

      // Get the most recent message and attach sender profile
      const sortedMsgs = Array.isArray(t.utromail_messages)
        ? [...t.utromail_messages].sort((a: any, b: any) =>
            new Date(b.sent_at).getTime() - new Date(a.sent_at).getTime()
          )
        : t.utromail_messages ? [t.utromail_messages] : [];
      const lastMsg = sortedMsgs[0] || null;
      const lastMessageWithSender = lastMsg
        ? {
            ...lastMsg,
            sender_name: lastMsg.user_profiles?.display_name || lastMsg.user_profiles?.username || null,
            sender_username: lastMsg.user_profiles?.username || null,
            sender_avatar: lastMsg.user_profiles?.avatar_url || null,
          }
        : null;
