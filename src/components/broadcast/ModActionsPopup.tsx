    ...
    try {
      ...
      if (isTargetHost) {
        const { error: profileError } = await supabase
          .from('user_profiles')
          .update({ broadcast_mic_muted: true })
          .eq('id', targetUserId);
        
        if (profileError) {
          console.error('[ModActions] Failed to mute host mic:', profileError);
        }
      } else {
        ...
        if (participantError) {
          console.error('[ModActions] Failed to mute participant:', participantError);
        }
      }
  ...
  const handleUnmute = async () => {
    ...
    try {
      ...
      if (isTargetHost) {
        await supabase
          .from('user_profiles')
          .update({ broadcast_mic_muted: false })
          .eq('id', targetUserId);
      } else {
        await supabase
          .from('stream_participants')
          .update({ mic_muted: false })
          .eq('stream_id', streamId)
          .eq('user_id', targetUserId);
      }
  ...
  const handleKick = async () => {
    ...
    try {
      if (streamId) {
        ...
        // Ban from stream to prevent re-joining (expires in 24 hours)
        const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
        await supabase.from('stream_bans').insert({
          stream_id: streamId,
          user_id: targetUserId,
          banned_by: currentUserId,
          reason: 'Kicked and banned from stream',
          expires_at: expiresAt,
        });
      }
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed left-1/2 top-[15%] -translate-x-1/2 -translate-y-0 w-[400px] max-h-[75vh] bg-slate-900 border border-slate-700 rounded-2xl overflow-hidden z-50 shadow-2xl"
          ...
          {activeTab === 'gift' ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              {MOD_ACTIONS.map((action) => {
                ...
                const streamRequiredActions = ['mute', 'unmute', 'kick', 'end_stream'];
                const needsStream = streamRequiredActions.includes(action.id);
                const noStream = !streamId;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-xs text-slate-400">
                      {needsStream && noStream ? 'No active stream' : action.description}
                    </div>
