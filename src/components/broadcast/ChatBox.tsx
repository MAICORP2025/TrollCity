...
export default function ChatBox({ streamId, onProfileClick, onCoinSend }: ChatBoxProps) {
  ...
  const [userCache, setUserCache] = useState<Record<string, { username: string; perks: string[]; hasInsurance?: boolean; rgbExpiresAt?: string; avatar_url?: string }>>({});
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          let profile = userCache[newMsg.user_id];
          if (!profile) {
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <button
                  ...
                  className={`flex items-center gap-1 ${getUsernameStyle(msg.sender_profile?.perks, msg.sender_profile?.rgbExpiresAt)}`}
                >
