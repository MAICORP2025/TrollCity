...
import { Eye, Users, Dice5, Crown, DollarSign } from 'lucide-react';
import { cn } from '@/lib/utils';
import type { TrollopolyMatch, TrollopolyPlayer } from '@/lib/game/gameTypes';
    ...
    >
      ...
      <div className="p-3">
        ...
        <div className="space-y-1">
          ...
          {/* Empty slots */}
          {Array.from({ length: Math.max(0, 6 - match.players.length) }).map((_, i) => (
        ...
        <p className="text-[10px] text-white/40 text-center mt-3">
          {match.players.length}/6 players • Min 2 to start
        </p>
      </div>
    </motion.div>
  );
