...
export default function BattleGiftPanel({
    ...
    const fetchGifts = async () => {
      try {
        const { data, error } = await supabase
          .from('gift_items')
          .select('id, name, icon, value, animation_type, gift_slug, category')
  ...
  const handleSendGift = useCallback(async (gift: GiftItem) => {
    ...
    try {
      const success = await sendGift(gift, { receiverId: recipientId, quantity: 1, battleId });
      if (success) {
