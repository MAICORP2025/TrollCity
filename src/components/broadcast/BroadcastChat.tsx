...
export default function BroadcastChat({ 
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    if (streamSettings?.paid_chat_enabled) {
      ...
      if (!paidAccess) {
        const price = streamSettings.paid_chat_price || 100;
        const chargeType = streamSettings.paid_chat_type === 'per_chat' ? 'per message' : 'for this stream';
        
        if (streamSettings.paid_chat_type === 'per_chat') {
          // Charge per message - deduct coins now
          if (profile.troll_coins < price) {
            toast.error(`Not enough coins. You need ${price} coins to send a message.`);
            setInput(content);
            return;
          }
          ...
          const { error: deductError } = await supabase
            .from('user_profiles')
            .update({ troll_coins: profile.troll_coins - price })
            .eq('id', user.id);
          ...
          toast.success(`Paid ${price} coins for chat access`);
        } else {
          // Per user - prompt to pay once
          toast.error(`This stream has paid chat. Pay ${price} coins ${chargeType} to participate.`);
          setInput(content);
          return;
        }
      }
