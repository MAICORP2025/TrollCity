...
export default function BattleView({ battleId, currentStreamId, viewerId, localTracks: passedLocalTracks, remoteUsers: passedRemoteUsers, userIdToLiveKitIdentity, onReturnToStream }: BattleViewProps) {
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-slate-900 via-purple-950 to-slate-900 overflow-hidden z-50">
      ...
      {/* Global Battle Score Overlay - Always centered at top, independent of grid */}
      <div 
      ...
      >
        <div className={cn(
          ...
          isSuddenDeath 
            ? "bg-red-950/90 border-red-500/60 shadow-red-500/30" 
            : "bg-black/80 border-white/20"
        )}>
          {/* Battle Mode Label */}
          <span className={cn(
            "text-xs md:text-sm font-black uppercase tracking-wider mb-1",
          ...
          {/* Timer */}
          <div className={cn(
            "font-mono text-lg md:text-2xl font-black mb-1",
          ...
          <div className="flex items-center justify-center gap-3 md:gap-6">
            ...
            <div className="flex flex-col items-center">
              <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-purple-400 truncate max-w-[60px] md:max-w-[80px]">
                {challengerStream.title?.slice(0, 8) || 'Team 1'}
              </span>
              <div className="font-mono text-lg md:text-2xl font-black text-purple-400">
            ...
            {/* Team 2 (Opponent) */}
            <div className="flex flex-col items-center">
              <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-emerald-400 truncate max-w-[60px] md:max-w-[80px]">
                {opponentStream.title?.slice(0, 8) || 'Team 2'}
              </span>
              <div className="font-mono text-lg md:text-2xl font-black text-emerald-400">
          ...
          {/* Potential Rewards */}
          <div className="flex items-center justify-center gap-3 mt-1 text-[9px] md:text-[10px] text-yellow-400/80">
            <div className="flex items-center gap-0.5">
              <Coins size={8} />
              <span>+{Math.round((battle?.score_challenger || 0) * 0.1)}</span>
            </div>
            <div className="flex items-center gap-0.5">
              <Coins size={8} />
              <span>+{Math.round((battle?.score_opponent || 0) * 0.1)}</span>
            </div>
          </div>
</div>
        </div>
