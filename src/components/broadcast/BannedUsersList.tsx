...
interface BannedUsersListProps {
    streamId: string;
    onClose: () => void;
    activeViewersOverride?: ActiveViewer[];
}

export default function BannedUsersList({ streamId, onClose, activeViewersOverride }: BannedUsersListProps) {
    ...
    const fetchActiveViewers = useCallback(async () => {
        if (activeViewersOverride) {
            setActiveViewers(activeViewersOverride);
            return;
        }
        ...
        try {
            ...
            const viewers = data.map(v => {
                ...
                return {
                    ...
                    joined_at: v.joined_at || (v as any).last_seen || new Date().toISOString()
                };
            });
