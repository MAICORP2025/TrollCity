...
export default function PodcastLockdownControl() {
  const { isLocked: locked, loading, toggleLockdown } = usePodcastLockdown();
  ...
  const handleToggleLockdown = async () => {
    ...
    try {
      ...
      toast.success(newState ? 'Podcasts Disabled' : 'Podcasts Enabled');
    } catch (error) {
      console.error('Error toggling podcast lockdown:', error);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-4 bg-slate-900/50 rounded-xl border border-white/10">
        ...
        <span className="ml-2 text-slate-400">Loading podcast status...</span>
      </div>
    );
  }
    ...
    `}>
      <div className="p-6 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className={`text-lg font-bold ${locked ? 'text-red-400' : 'text-green-400'}`}>
              Podcast Lockdown: {locked ? 'ACTIVE' : 'INACTIVE'}
            </h3>
            <p className="text-sm text-slate-400">
              {locked 
                ? 'All podcasts are disabled. No one can start a podcast.' 
                : 'All users can start podcasts normally.'}
            </p>
          </div>
        </div>
        ...
        >
          ...
          {locked ? 'Enable Podcasts' : 'Disable Podcasts'}
        </button>
      </div>
      ...
      {locked && (
        <div className="px-6 pb-4 flex items-center gap-2 text-xs text-red-300/70">
          ...
          Note: Existing podcasts may not be immediately terminated, but new ones cannot start.
        </div>
      )}
    </div>
  );
