...
export default function HytroGamingLockdownControl() {
  const { isLocked: locked, loading, toggleLockdown } = useHytroGamingLockdown();
  ...
  const handleToggleLockdown = async () => {
    ...
    try {
      ...
      toast.success(newState ? 'HytroGaming Disabled' : 'HytroGaming Enabled');
    } catch (error) {
      console.error('Error toggling HytroGaming lockdown:', error);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-4 bg-slate-900/50 rounded-xl border border-white/10">
        ...
        <span className="ml-2 text-slate-400">Loading HytroGaming status...</span>
      </div>
    );
  }
    ...
    `}>
      <div className="p-6 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className={`text-lg font-bold ${locked ? 'text-red-400' : 'text-green-400'}`}>
              HytroGaming Lockdown: {locked ? 'ACTIVE' : 'INACTIVE'}
            </h3>
            <p className="text-sm text-slate-400">
              {locked 
                ? 'All HytroGaming streaming is disabled. No one can go live.' 
                : 'All users can stream HytroGaming normally.'}
            </p>
          </div>
        </div>
        ...
        >
          ...
          {locked ? 'Enable HytroGaming' : 'Disable HytroGaming'}
        </button>
      </div>
