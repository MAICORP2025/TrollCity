...
const isActive = false;
  ...
  // Show highlight once per user - use localStorage to track if already seen
  const [showHighlight, setShowHighlight] = useState(false)
  useEffect(() => {
    const hasSeenNavHighlight = localStorage.getItem('has_seen_nav_highlight')
    if (!hasSeenNavHighlight) {
  ...
  const handleLogout = async () => {
    try {
      // Set flag BEFORE signOut to prevent session recovery
      ...
      try {
        localStorage.clear()
        // Preserve trollIntroSeen across logout
  ...
  const canBroadcast = () => {
    return !isBroadcastLockedDown &&
      ...
      ((profile as any)?.role === 'broadcaster' || (profile as any)?.is_broadcaster || (profile as any)?.troll_role === 'broadcaster')
  }
  ...
  const getRoleInfo = () => {
    ...
    if ((role as any) === 'broadcaster') return { label: 'Broadcaster', icon: Video, color: 'from-purple-500 to-blue-500' }
...
// City Registry
  ...
  // Menu Options - Role-based, sorted with Coin Store and Home at top
  const getMenuOptions = () => {
    ...
    return allOptions.sort((a, b) => {
      if (a.label === "Coin Store") return -1
      if (b.label === "Coin Store") return 1
        ...
        {isBubbleVisible && isMobile && (
          ...
          >
             <motion.button
               ...
               className={`w-14 h-14 rounded-full bg-gradient-to-tr ${roleInfo.color} p-[2px] shadow-[0_0_25px_rgba(124,58,237,0.5)] hover:shadow-[0_0_35px_rgba(124,58,237,0.7)] transition-shadow duration-300 relative`}
             >
              {/* New user highlight ring */}
              {showHighlight && (
                <motion.div
            ...
            {showHighlight && (
              <div className="absolute -top-12 left-1/2 -translate-x-1/2 whitespace-nowrap">
                <div className="bg-yellow-500/90 text-black text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg">
                  Tap to explore!
                </div>
                <div className="absolute left-1/2 -translate-x-1/2 -bottom-1.5 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent border-t-yellow-500/90" />
              </div>
            )}
              ...
              <div className="p-4 max-w-4xl mx-auto">
                {!user ? (
                  ...
                  <div className="space-y-6 mt-4">
                    ...
                    {/* Balance Summary */}
                    <div className="bg-gradient-to-r from-slate-800/50 to-slate-900/50 rounded-xl border border-slate-700/50 p-4">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="text-center">
                          <div className="text-xs text-slate-400 uppercase tracking-wide">Troll Coins</div>
                          <div className="text-lg font-bold text-yellow-400">{profile?.troll_coins?.toLocaleString() || '0'}</div>
                        </div>
                        <div className="text-center">
                          <div className="text-xs text-slate-400 uppercase tracking-wide">Trollmonds</div>
                          <div className="text-lg font-bold text-purple-400">{profile?.trollmonds?.toLocaleString() || '0'}</div>
                        </div>
                        <div className="text-center">
                          <div className="text-xs text-slate-400 uppercase tracking-wide">Level</div>
                          <div className="text-lg font-bold text-cyan-400">{profile?.level || '1'}</div>
                        </div>
                        <div className="text-center">
                          <div className="text-xs text-slate-400 uppercase tracking-wide">XP</div>
                          <div className="text-lg font-bold text-green-400">{profile?.xp?.toLocaleString() || '0'}</div>
                        </div>
                      </div>
                      {profile?.role && (
                        <div className="mt-3 pt-3 border-t border-slate-600/50">
                          <div className="text-center">
                            ...
                            <div className="text-sm font-bold text-white capitalize">{profile.role.replace('_', ' ')}</div>
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    {/* Quick Access - Home, Search & Coin Store */}
                    <div className="grid grid-cols-3 gap-3">
                      <Link
                        to="/"
                        onClick={() => setIsMenuOpen(false)}
                        className="flex flex-col items-center gap-2 p-4 bg-gradient-to-br from-blue-600/[0.15] to-cyan-600/[0.15] hover:from-blue-600/[0.25] hover:to-cyan-600/[0.25] rounded-xl border border-blue-500/[0.3] transition-all duration-200 group"
                      >
                        <div className="p-2.5 rounded-lg bg-blue-500/[0.2] text-blue-400 group-hover:bg-blue-500/[0.3] transition-colors">
                          <Home size={24} />
                        </div>
                      ...
                      <Link
                        to="/search"
                        onClick={() => setIsMenuOpen(false)}
                        className="flex flex-col items-center gap-2 p-4 bg-gradient-to-br from-green-600/[0.15] to-emerald-600/[0.15] hover:from-green-600/[0.25] hover:to-emerald-600/[0.25] rounded-xl border border-green-500/[0.3] transition-all duration-200 group"
                      >
                        <div className="p-2.5 rounded-lg bg-green-500/[0.2] text-green-400 group-hover:bg-green-500/[0.3] transition-colors">
                          <Search size={24} />
                        </div>
                      ...
                      <Link
                        to="/store"
                        onClick={() => setIsMenuOpen(false)}
                        className="flex flex-col items-center gap-2 p-4 bg-gradient-to-br from-yellow-600/[0.15] to-amber-600/[0.15] hover:from-yellow-600/[0.25] hover:to-amber-600/[0.25] rounded-xl border border-yellow-500/[0.3] transition-all duration-200 group"
                      >
                        <div className="p-2.5 rounded-lg bg-yellow-500/[0.2] text-yellow-400 group-hover:bg-yellow-500/[0.3] transition-colors">
                          <Coins size={24} />
                        </div>
                        <span className="text-white font-bold text-sm">Coin Store</span>
                      </Link>
                    </div>
                    ...
                    {Object.entries(
                      ...
                      <div key={category}>
                        ...
                        <div className="grid grid-cols-4 md:grid-cols-2 gap-2">
                          {options.filter((opt: any) => opt.label !== 'Home' && opt.label !== 'Coin Store' && opt.label !== 'Search').map((opt: any, i: number) => {
                            const OptIcon = opt.icon
                            const isTutorialTarget = isActive && targetPage?.path === opt.path
                            return (
                              <Link
                                key={i}
                                to={opt.path}
                                ...
                                className={`flex flex-col items-center gap-1 p-2 rounded-lg border transition-all duration-200 group ${
                                  isTutorialTarget 
                                    ? 'ring-2 animate-pulse bg-pink-500/10 border-pink-500/50' 
                                    : 'bg-white/[0.03] hover:bg-white/[0.06] border-white/[0.05] hover:border-white/[0.1]'
                                }`}
                                style={isTutorialTarget ? {
                                  boxShadow: '0 0 20px rgba(255, 0, 127, 0.6), 0 0 40px rgba(0, 200, 255, 0.4), inset 0 0 15px rgba(255, 0, 127, 0.1)'
                                } : {}}
                              >
                                <div className={`p-1.5 rounded-lg transition-colors ${
                                  isTutorialTarget 
                                    ? 'bg-pink-500/20 text-pink-400' 
                                    : 'bg-purple-500/[0.08] text-purple-400 group-hover:bg-purple-500/[0.14]'
                                }`}>
                                  <OptIcon size={16} />
                                </div>
                                <span className={`font-medium text-[10px] flex-1 text-center line-clamp-1 ${
                                  isTutorialTarget ? 'text-pink-300' : 'text-white'
                                }`}>{opt.label}</span>
                                {opt.badge > 0 && (
