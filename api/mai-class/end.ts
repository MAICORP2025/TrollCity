...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!profile || (profile.role !== 'admin' && profile.role !== 'owner' && profile.role !== 'ceo' && !profile.is_admin)) {
      return res.status(403).json({ error: 'Only admins can end class' })
    }
    ...
    // Verify class exists and user is instructor
    ...
    if (existingClass.instructor_id !== userId) {
      return res.status(403).json({ error: 'Only the instructor can end this class' })
    }
