# Troll City → MaiTalent.fun Verification Update

Hi MaiTalent team,

I reviewed the current Troll City implementation against the new integration prompt and here is the current status.

## Current status

The core server-side integration hooks are already present in the Troll City codebase:
...
1. Server-side sync execution
   - Link and sync requests are initiated from the Troll City backend rather than the frontend.
...
3. Profile-based balance usage
   - The frontend balance flow already reads from the Troll City profile record for troll_coins, hype_coins, and battle_crowns through [src/lib/hooks/useCoins.ts](src/lib/hooks/useCoins.ts).
...
2. Explicit balance-source-of-truth sync payloads
   - The current server-side sync flow is in place, but the outbound payload should explicitly reflect the Troll City profile balances as the authoritative values for the link/sync flow.
...
1. Add the explicit profile-link state fields to Troll City profile storage and update them during successful link/sync operations.
2. Extend the outbound sync payload so that Troll City profile balances are clearly treated as the single source of truth for MaiTalent balance syncs.
