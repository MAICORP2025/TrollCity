  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'support_tickets') {
        ...
        const isOfficer = profile?.is_troll_officer || profile?.is_lead_officer || profile?.role === 'troll_officer' || profile?.role === 'lead_troll_officer';

        if (!isAdmin && !isSecretary && !isOfficer) {
          return new Response(JSON.stringify({ error: 'Unauthorized' }), {
            status: 403,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        ...
        if (cmd === 'resolve') {
          ...
          if (!ticketId || !response) {
            return new Response(JSON.stringify({ error: 'Missing ticketId or response' }), {
              status: 400,
              headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            });
          }
