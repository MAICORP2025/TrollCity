  ...
  try {
    ...
    switch (action) {
       case 'accept_agreement': {
         ...
         // Record agreement acceptance (this also updates user_profiles.terms_accepted)
         const { data: agreementId, error: agreementError } = await supabaseClient.rpc('record_agreement_acceptance', {
           p_user_id: user.id,
         ...
         if (agreementError) {
           ...
           return new Response(
             JSON.stringify({ success: false, error: agreementError.message }),
             {
               status: 500,
               headers: { ...corsHeaders, 'Content-Type': 'application/json' }
             }
         ...
         if (profileError) {
           ...
           return new Response(
             JSON.stringify({
               ...
               message: 'Agreement accepted, but profile refresh failed'
             }),
             {
               status: 200,
               headers: { ...corsHeaders, 'Content-Type': 'application/json' }
