...
VALUES
  ...
  ('AR Troll Mask', 1000, '🎭', 'ar_troll_mask', 'ar_face', 'ar_troll_mask', 'AR Face Gifts', 'rare', 'Animated Troll City face mask overlay', true, 15),
...
INSERT INTO gift_items (name, value, icon, gift_slug, animation_type, animation_key, category, rarity, description, is_active, duration)
VALUES
  ('AR Mini Troll', 1000, '🧌', 'ar_mini_troll', 'ar_troll', 'ar_mini_troll', 'AR Troll City', 'rare', 'Tiny Troll City mascot sits on shoulder', true, 15),
  ('AR Troll Army', 100000, '🎪', 'ar_troll_army', 'ar_troll', 'ar_troll_army', 'AR Troll City', 'mythic', 'Multiple troll mascots march across stream', true, 15),
  ('AR Troll City President', 1000000, '🏰', 'ar_troll_city_president', 'ar_legendary', 'ar_troll_city_president', 'AR Legendary', 'mythic', 'LEGENDARY: President Mansion, crown, helicopter, particle storm', true, 20)
ON CONFLICT (gift_slug) DO UPDATE SET
  value = EXCLUDED.value,
  icon = EXCLUDED.icon,
  animation_type = EXCLUDED.animation_type,
...
BEGIN
  ...
  v_total_cost := v_gift.value * p_quantity;
