...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM public.gift_items WHERE name = 'Diamond Ring') THEN
        INSERT INTO public.gift_items (name, value, icon, animation_type, currency, category) VALUES ('Diamond Ring', 10, '💍', 'tier_1', 'trollmonds', 'Luxury');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM public.gift_items WHERE name = 'Briefcase') THEN
        INSERT INTO public.gift_items (name, value, icon, animation_type, currency, category) VALUES ('Briefcase', 50, '💼', 'tier_2', 'trollmonds', 'Luxury');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM public.gift_items WHERE name = 'Luxury Car Key') THEN
        INSERT INTO public.gift_items (name, value, icon, animation_type, currency, category) VALUES ('Luxury Car Key', 100, '🔑', 'tier_2', 'trollmonds', 'Luxury');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM public.gift_items WHERE name = 'Mansion Deed') THEN
        INSERT INTO public.gift_items (name, value, icon, animation_type, currency, category) VALUES ('Mansion Deed', 500, '📜', 'tier_3', 'trollmonds', 'Luxury');
    END IF;
    IF NOT EXISTS (SELECT 1 FROM public.gift_items WHERE name = 'Private Jet') THEN
        INSERT INTO public.gift_items (name, value, icon, animation_type, currency, category) VALUES ('Private Jet', 1000, '✈️', 'tier_4', 'trollmonds', 'Luxury');
    END IF;
END $$;
