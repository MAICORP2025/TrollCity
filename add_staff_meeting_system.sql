...
BEGIN
  ...
  IF profile.role = 'admin' OR profile.is_admin = TRUE OR profile.is_superadmin = TRUE THEN
    RETURN TRUE;
  END IF;
  ...
  -- Troll officer roles
  IF profile.is_troll_officer = TRUE OR profile.role = 'troll_officer' OR profile.troll_role = 'troll_officer' OR profile.officer_role = 'troll_officer' THEN
    RETURN TRUE;
  END IF;
  
  -- Pastor role
  IF profile.is_pastor = TRUE OR profile.role = 'pastor' THEN
    RETURN TRUE;
  END IF;
  
  -- Secretary role
  IF profile.is_secretary = TRUE OR profile.role = 'secretary' THEN
    RETURN TRUE;
  END IF;
  
  -- Prosecutor role
  IF profile.is_prosecutor = TRUE OR profile.role = 'prosecutor' THEN
    RETURN TRUE;
  END IF;
  
  -- Auctioneer role
  IF profile.is_auctioneer = TRUE OR profile.role = 'auctioneer' THEN
    RETURN TRUE;
  END IF;
