...
import { Radio, Users, Play, Eye, X, ChevronRight, Link2, Sparkles } from 'lucide-react'
...
type TabType = 'live' | 'wall' | 'universe'
  ...
  const handleLiveItemClick = (item: LiveItem) => {
   ...
   return (
     <div className={`relative h-dvh flex flex-col overflow-hidden ${trollCityTheme.backgrounds.primary}`}>
       {/* Loading Overlay GÇö appears during auth init without unmounting content */}
       {isLoading && (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                    {liveItems.filter(i => i.isBattle).map((item) => (
                      <div
                        key={item.id}
                        onClick={() => handleLiveItemClick(item)}
                        className="relative bg-gradient-to-br from-yellow-900/40 to-orange-900/40 rounded-xl overflow-hidden cursor-pointer group hover:ring-2 hover:ring-yellow-400/50 transition-all"
                      >
                        <div className="absolute inset-0 flex items-center justify-center">
                          {item.streamerAvatar ? (
                            <img src={item.streamerAvatar} alt={item.streamerName} className="w-full h-full object-cover opacity-50" />
                          ) : (
                        ...
                        <div className="relative p-3">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className="text-slate-400 text-xs">GÇó</span>
        ...
        <div className="flex-shrink-0 px-4 py-6 bg-slate-950/80 border-t border-slate-800">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-wrap items-center justify-center gap-4 text-xs">
              ...
              <span className="text-slate-600">GÇó</span>
